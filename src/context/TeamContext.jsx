...
export function TeamProvider({ children }) {
  ...
  // ✅ FIX: lazy initializer reads localStorage ONCE at mount, synchronously.
  // No load effect needed — state is correct from the very first render.
  const [teams, setTeams] = useState(() =>
    safeGetFromStorage("teams_db", [])
  );

  // ✅ FIX: a ref that starts false and is flipped to true after the FIRST
  // committed render (inside a useEffect with no deps). This guarantees the
  // save effect never fires on the initial render with the empty default.
  ...
  // Save on every subsequent change — skips the very first render.
  useEffect(() => {
  ...
  const getMyTeams = () =>
    teams.filter((t) => t.members.some((m) => m.userId === user?.id));
  ...
  const deleteTeam = (teamId) => {
    ...
    safeSetToStorage(
      "projects_db",
      projects.map((p) => (p.teamId === teamId ? { ...p, teamId: null } : p))
    );
  ...
  const inviteMember = (teamId, targetUser) => {
    ...
    if (!foundUser)
      throw new Error(
        "No account found for this user. They must sign up first."
      );
  ...
  const removeMember = (teamId, targetUserId) => {
    ...
    if (targetUserId === user.id)
      throw new Error("You cannot remove yourself. Use Leave Team.");
