...
export function ProjectProvider({ children }) {
  ...
  // ✅ FIX: lazy initializer — reads localStorage synchronously at mount.
  const [projects, setProjects] = useState(() =>
    safeGetFromStorage("projects_db", [])
  );
  ...
  // Save only after the first committed render.
  useEffect(() => {
      ...
      if (team) {
        team.members.forEach((member) => {
          if (member.userId !== user.id) {
            const exists = initialMembers.some(
  ...
  const addMember = (projectId, newMember) => {
    ...
    setProjects((prev) => {
      ...
      if (memberRole === "owner")
        throw new Error(
          "Cannot add another owner. Use role transfer instead."
        );
